...
const Sell = () => {
  ...
  const [sell_price, setPrice] = useState(0);
  ...
  const handleSell = async () => {
    try {
      const response = await fetch('http://localhost:3001/Sell', {
        ...
        body: JSON.stringify({ comp_select, qty, sell_price }),
      });
      ...
      if (response.ok) {
        ...
        console.error('Failed to Sell. Reason:', responseBody2.reason);
    ...
    console.log('Selling:', { comp_select, qty, sell_price });
  };

  return (
    <div style={homePageStyle}>
      <div style={containerStyle}>
        <h2 style={headingStyle}>Stock Selling Form</h2>
        ...
        <div>
          <label htmlFor="SellPrice" style={labelStyle}>
            Sell Price:
          </label>
          <input
            type="number"
